...
class diff_helper{
  ...
  private static function getCellContent(
    ...
    while ($index < count($diff) && $diff[$index][1] == $type){
      $html .=
          '<span><font class="linenumber">'.($index+1).'</font> '
...
$("body").keydown(function (event) {
		...
		if (event.which == 13 ) {
			...
			if(scrollto_idx >= $(".diffDeleted, .diffInserted").length)
				scrollto_idx = 0;
			
			elm = $(".diffDeleted, .diffInserted").get(scrollto_idx)
