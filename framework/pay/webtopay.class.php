...
class WebToPay {
	...
	public static function redirectToPayment($data, $exit = false) {
		...
		printf(
			'Redirecting to <a href="%s">%s</a>. Please wait.', htmlentities($url, ENT_QUOTES, 'UTF-8'), htmlentities($url, ENT_QUOTES, 'UTF-8')
		);
		if ($exit) {
			exit();
		}
	...
	public static function getPaymentUrl($language = 'LIT') {
		return (in_array($language, array('lt', 'lit', 'LIT'))) ? self::PAY_URL : self::PAYSERA_PAY_URL;
	}
...
class WebToPay_CallbackValidator {
	...
	public function validateAndParseData(array $requestData) {
		...
		if (!isset($request['projectid'])) {
			...
			'Project ID not provided in callback', WebToPayException::E_INVALID
			);
		}

		if ((string) $request['projectid'] !== (string) $this->projectId) {
			throw new WebToPay_Exception_Callback(
			sprintf('Bad projectid: %s, should be: %s', $request['projectid'], $this->projectId), WebToPayException::E_INVALID
			);
		}

		if (!isset($request['type']) || !in_array($request['type'], array('micro', 'macro'))) {
			$micro = (
				isset($request['to']) && isset($request['from']) && isset($request['sms'])
				);
...
class WebToPay_PaymentMethodGroup {
	...
	$key, $minAmount, $maxAmount, $currency, array $logoList = array(), array $titleTranslations = array(), $isIban = false, $baseCurrency = null
	) {
		return new WebToPay_PaymentMethod(
			$key, $minAmount, $maxAmount, $currency, $logoList, $titleTranslations, $this->defaultLanguage, $isIban, $baseCurrency
		);
	}
...
class WebToPay_SmsAnswerSender {
	...
	$password, WebToPay_WebClient $webClient, WebToPay_UrlBuilder $urlBuilder
	) {
	...
	public function sendAnswer($smsId, $text) {
		...
		if (strpos($content, 'OK') !== 0) {
			throw new WebToPayException(
			sprintf('Error: %s', $content), WebToPayException::E_SMS_ANSWER
			);
		}
	}
...
class WebToPay_PaymentMethod {
	 ...
	 */
	public function __construct(
	$key, $minAmount, $maxAmount, $currency, array $logoList = array(), array $titleTranslations = array(), $defaultLanguage = 'lt', $isIban = false, $baseCurrency = null
	) {
		$this->key = $key;
	...
	public function isAvailableForAmount($amount, $currency) {
		...
		return (
			($this->minAmount === null || $amount >= $this->minAmount) && ($this->maxAmount === null || $amount <= $this->maxAmount)
			);
	}
...
class WebToPay_RequestBuilder {
	 ...
	 */
	public function __construct(
	$projectId, $projectPassword, WebToPay_Util $util, WebToPay_UrlBuilder $urlBuilder
	) {
		$this->projectId = $projectId;
	...
	protected function validateRequest($data, $specs) {
		foreach ($specs as $spec) {
			...
			if ($required && !isset($data[$name])) {
				...
				sprintf("'%s' is required but missing.", $name), WebToPayException::E_MISSING, $name
				);
			}
			...
			if (!empty($data[$name])) {
				if ($maxlen && strlen($data[$name]) > $maxlen) {
					throw new WebToPay_Exception_Validation(sprintf(
						"'%s' value is too long (%d), %d characters allowed.", $name, strlen($data[$name]), $maxlen
					), WebToPayException::E_MAXLEN, $name);
				}

				if ($regexp !== '' && !preg_match($regexp, $data[$name])) {
					throw new WebToPay_Exception_Validation(
					sprintf("'%s' value '%s' is invalid.", $name, $data[$name]), WebToPayException::E_REGEXP, $name
					);
				}
			}
		}
...
class WebToPay_PaymentMethodListProvider {
	 ...
	 */
	public function __construct(
	$projectId, WebToPay_WebClient $webClient, WebToPay_UrlBuilder $urlBuilder
	) {
		$this->projectId = $projectId;
		$this->webClient = $webClient;
		$this->urlBuilder = $urlBuilder;
	...
	public function getCallbackValidator() {
		if ($this->callbackValidator === null) {
			...
			$this->callbackValidator = new WebToPay_CallbackValidator(
				$this->configuration['projectId'], $this->getSigner(), $this->getUtil()
			);
		}
	...
	public function getRequestBuilder() {
		if ($this->requestBuilder === null) {
			...
			$this->requestBuilder = new WebToPay_RequestBuilder(
				$this->configuration['projectId'], $this->configuration['password'], $this->getUtil(), $this->getUrlBuilder()
			);
		}
	...
	public function getUrlBuilder() {
		if ($this->urlBuilder === null) {
			$this->urlBuilder = new WebToPay_UrlBuilder(
				$this->configuration, $this->environment
			);
		}
	...
	public function getSmsAnswerSender() {
		if ($this->smsAnswerSender === null) {
			...
			$this->smsAnswerSender = new WebToPay_SmsAnswerSender(
				$this->configuration['password'], $this->getWebClient(), $this->getUrlBuilder()
			);
		}
	...
	public function getPaymentMethodListProvider() {
		if ($this->paymentMethodListProvider === null) {
			...
			$this->paymentMethodListProvider = new WebToPay_PaymentMethodListProvider(
				$this->configuration['projectId'], $this->getWebClient(), $this->getUrlBuilder()
			);
		}
