...
class GW_User extends GW_Data_Object {
    ...
    function setValidators($set) {
        ...
        $validators_def =
                Array(
                    ...
                    'mob_phone' => Array('gw_phone', Array('min_length' => 6, 'max_length' => 20)),
        ...
        $validators_set = Array
            ...
            'insert' => Array('first_name', 'second_name', 'unique_email', 'email', 'pass_new', 'pass_new_repat', 'phone', 'license', 'mob_phone'),
            'update_info' => Array('first_name', 'second_name', 'phone', 'mob_phone', 'address', 'post_index', 'city', 'news'),
        );
    ...
    function validate() {
        ...
        if ($this->validators['unique_email'])
            if ($this->count(Array('email=? AND !removed', $this->get('email'))))
                $this->errors['email'] = '/USER/EMAIL_TAKEN';
    ...
    function invertBanned() {
        if (!$this->loaded)
            $this->load('banned');

        if (!$this->loaded)
            return false;
        $this->set('banned', (bool) $this->get('banned') ? 0 : 1);
        $this->update(Array('banned'));
