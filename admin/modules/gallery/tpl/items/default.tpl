...
<tr>
	<th></th>
	<th>{FH::fieldTitle(title)}</th>
	<th>{FH::fieldTitle(insert_time)}</th>
	<th>{FH::fieldTitle(update_time)}</th>
	<th>{$lang.ACTIONS}</th>
</tr>
...
<tr {if $smarty.get.id==$id}class="gw_active_row"{/if}>
	...
	<td valign="center">
		{if $item->type==1}
			<img src="img/icons/folder.png" align="absmiddle" vspace="2" />
		{else}
			{$image=$item->image}
			<img src="tools/img.php?id={$image->id}&size={$thumbn_sz}" align="absmiddle" vspace="2" />
		{/if}
		...
		<table>
			<tr><td align="center" style="border:1px solid silver;height:136px">
				{if $item->type==1}{*folder*}
					{$src="img/icons/folder_128x128.png"}
					{$link=FH::gw_path([params=>[pid=>$item->id]])}
					
					{include file="`$m->tpl_dir`/folder_icon.tpl"}
					
				{else}{*image*}
					{$image=$item->image}
					{$src="tools/img.php?id=`$image->id`&size=128x128"}
					{$link=FH::gw_path([params=>[id=>$item->id], relative_path=>"`$item->id`/form"])}
