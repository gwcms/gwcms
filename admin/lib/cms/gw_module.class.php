...
class GW_Module
{
	...
	function init()
	{
		...
		$this->lang = GW_Lang_XML::load("{$this->module_dir}lang.xml", GW::$request->ln);
	...
	function processTemplate($soft=1)
	{
		...
		if(file_exists($tmp = $file.'.php')){
			...
			if(!file_exists($tmp = $file.'.tpl'))
				...
				if(!file_exists($tmp = GW::$dir['MODULES']."default/tpl/".$basename.'.tpl'))
