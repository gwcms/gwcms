...
class Module_Repository extends GW_Common_Module {
	...
	function getListConfig() {
		...
		//d::dumpas($this->model->getColumns());
	...
	function __eventAfterList() {
		...
		$this->tpl_vars['icosize'] = $this->modconfig->icosize ?: '30x24';
	}
	...
	function __eventBeforeDelete($item) {
		...
		if($item->type == 'image'){
			$this->clearCachedIMResizetool($item);	
		}
	}
	...
	function doRotate() {
		...
		$im = new GW_Image_Manipulation($item->path);
	...
	function clearCachedIMResizetool($item) {
		GW::s('DIR/SYS_IMAGES_CACHE_1', $cachedir = GW::s('DIR/SYS_REPOSITORY') . 'cache/images_1/');

		//d::dumpas([$item,GW::s('DIR/SYS_IMAGES_CACHE_1'),$item->path, md5($item->path)]);

		$cached = glob(GW::s('DIR/SYS_IMAGES_CACHE_1') . md5($item->path) . '.*');

		foreach ($cached as $cachedim)
			unlink($cachedim);


		$after = glob(GW::s('DIR/SYS_IMAGES_CACHE_1') . md5($item->path) . '.*');
		$clearfound = count($cached);
		$clearfoundafter = count($after);

		$this->setMessage($msg="{$item->filename} Cleared cached $clearfound / $clearfoundafter");
	}
	...
	function listZipContents() {
		...
		if (!file_exists($zipFilePath)) {
			echo "File not found: $zipFilePath";
			return;
		}
		...
		if ($zip->open($zipFilePath) !== TRUE) {
			echo "Could not open ZIP file: $zipFilePath";
			return;
		}
	...
	function doPdfToImage()
	{
		$item = $this->getDataObjectById();

		$pdf = $item->path;
		
		$newim = preg_replace('/\.(pdf)$/i', '.webp', $pdf);

		$out = shell_exec($cmd = "convert -density 150 $pdf $newim");

		$this->setMessage("<pre>$out</pre>");
		$this->jump();
	}
