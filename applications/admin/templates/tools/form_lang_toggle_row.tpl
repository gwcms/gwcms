	...
	{foreach $langs as $ln_code}
		...
		<td>
			<span class="gw_switch_ln" href="#" onclick="tooglei18nCol('{$ln_code}');return false">
				<img src="{GW::s("STATIC_EXTERNAL_ASSETS")}flags/oneimgcss/blank.gif" class="flag flag-{$flag_code}" alt="{$ln_code}" /> <span class="toggle_i18n_{$ln_code}">{GW::l("/g/LANG/`$ln_code`")}</span>
			</span>
		</td>	
	{/foreach}
</tr>