...
{include file="{$smarty.current_dir}/actions.tpl"}
...
{function dl_proc_th_cell}
		...
		{else}
			...
			{if isset($dl_order_enabled_fields.$field)}
				{include file="{$smarty.current_dir}/order.tpl" name=$field title=$coltitle}
			{else}
				{$coltitle}
			{/if}
		{/if}
...
{else}
	...
	{if $m->paging_enabled && $m->list_params.paging_enabled && ($list || $m->list_params.page>1)}
		{$do_display_toolbar_pull_right[]=['file',"{$smarty.current_dir}/page_by.tpl"]}
	{/if}	

	{block name="open_tpl"}
		{include file="default_open.tpl"}
		
		{include file="common_module/tools/toolbar_buttons.tpl"}
		
		{call do_display_toolbar_buttons2}		
	{/block}


	{include file="{$smarty.current_dir}/output_filters.tpl"}	
	...
	{capture append=footer_hidden}
		<link href="{$app_root}templates/common_module/assets/css.css" rel="stylesheet" />
	{/capture}
	...
	<div class="row">
		...
		<div>
			{if !count($list)}
				<div class="gwcmsNoItems">{GW::ln('/g/NO_ITEMS')}</div>
			{else}
					<table class="table-condensed table-hover table-vcenter table-bordered gwTable gwActiveTable gwListTable" data-context="{get_class(current($list))}">
						<tr>	
							...
							{if !$smarty.get.print_view} {* show even no actions present*}
								<th><i class="fa fa-cog text-muted"></i> {*{call name="dl_actions_head"}*}</th>
							{/if}
						</tr>
