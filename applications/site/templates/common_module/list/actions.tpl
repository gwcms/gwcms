{function list_item_action}
	...
	<a  class="{$action_class|default:gwcmsAction} {$action_addclass} {if $shift_button}shiftbtn{/if}"		
		...
		><span class="{if $smallcap}gwactcapsmall{/if} {$captclass}">{if $iconclass}<i class="{$iconclass}"></i>{/if}{if $caption}{$caption}{/if}</span></a>
	{if $addlitag}</li>{/if}
{/function}

{function list_item_title}{strip}
	{if GW::ln("/g/VIEWS/{$searchkey}",[asis=>1])}
		{GW::ln("/g/VIEWS/{$searchkey}")}
	{elseif GW::ln("/m/VIEWS/{$searchkey}",[asis=>1])}
		{GW::ln("/m/VIEWS/{$searchkey}")}
	{else}
		{$searchkey}
	{/if}
{/strip}{/function}
...
{function name=dl_actions_delete}
	{if $m->canBeAccessed($item, [access=>$smarty.const.GW_PERM_WRITE,nodie=>1])}
		{list_item_action_m url=[false,[act=>doDelete,id=>$item->id]] iconclass="fa fa-trash-o text-danger" confirm=1 caption=GW::ln('/g/REMOVE') shift_button=1}
	{/if}
{/function}
...
{function name=dl_actions_edit}
	{list_item_action_m url=["`$app->idInPath($item->id)`/form"] iconclass="fa fa-pencil-square-o " 
		captclass="text-brown uppercase" caption=GW::ln('/g/EDIT')}
{/function}
...
{function dl_cl_actions_invertactive}
	<option value="checked_action('{$m->buildUri(false,[act=>doSeriesAct,action=>doInvertActive])}', 1)">{GW::ln('/A/VIEWS/doInvertActive')}</option>
{/function}

{function dl_cl_actions_dialogremove}
	<option value="checked_action('dialogremove')">{GW::ln('/A/VIEWS/dialogremove')}</option>
{/function}
