...
<tr><th>{GW::ln('/g/LDS_FIELD_PRIORITY_VISIBILITY')}</th><th>{GW::ln('/g/ORDERS_LABEL')}</th></tr>
...
<i id="cols_deactivated_label" style="color:brown">{GW::ln('/g/DEACTIVATED_FIELDS')}:</i>
<ul class="cols_deactivated">
</ul>
...
<div style='border-top: 1px solid rgba(0, 0, 0, 0.07);padding: 10px;'>
	...
	<select id="saveto" name="pageviewid" class="selectpicker">
		...
		{foreach $page_views as $pview}
			<option value="{$pview->id}" {if $current_page_view_id == $pview->id}selected="selected"{/if}>{GW::ln('/g/PAGE_VIEWS')}: {$pview->title}</option>
		{/foreach}
		{/if}
		
	</select>
	<button id="submitbtn" class="btn btn-primary" data-save="{GW::ln('/g/SAVE')}" data-apply="{GW::ln('/g/APPLY_1')}"></button>
</div>
...
require(["gwcms"], function(){	
	...
	$('.switchDefault').attr('title', "{GW::ln('/g/SET_AS_DEFAULT')}")
