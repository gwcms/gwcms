...
{function "page_view_a"}
	<a class="btn {if $m->list_config.pview->id==$view->id}btn-primary{else}btn-default{/if}" 
		...
		>
		{if $view->default}<i class="fa fa-home" title="{GW::ln('/g/PAGE_VIEW_DEFAULT')}"></i>{/if} 
...
<div class="btn-group">
	...
	<button class="btn btn-default dropdown-toggle" data-toggle="dropdown">
	    ...
	    <i class="fa fa-filter" aria-hidden="true" title="{GW::ln('/g/VIEWS_LABEL')}"></i>
	</button>
	<ul class="dropdown-menu">
	    <li><a class="iframeopen" href="{$m->buildUri(false,[act=>doCreatePageView,clean=>2])}">{GW::ln('/g/CREATE_NEW_VIEW')}</a></li>
	    {if $m->list_config.pview->id}
		<li><a class="iframeopen" href="{$m->buildUri(false,[act=>doCreatePageView,update=>1,clean=>2])}">{GW::ln('/g/UPDATE_CURRENT_VIEW')}</a></li>
	    {/if}
	    <li><a class="iframeopen" href="{$m->buildUri(false,[act=>doManagePageViews,clean=>2])}" title="{GW::ln('/M/SYSTEM/MAP/childs/page_views/title')}">{GW::ln('/g/MANAGE')}</a></li>
	</ul>
    </div>	
...
<div class="btn-group">
	...
	<button class="btn btn-default dropdown-toggle" data-toggle="dropdown">
	    ...
	    <i class="fa fa-sort-amount-asc" aria-hidden="true" title="{GW::ln('/g/VIEWS_LABEL')}"></i>
	</button>
	<ul class="dropdown-menu">
	    <li><a class="iframeopen" href="{$m->buildUri(false,[act=>doCreatePageView,clean=>2,saveasorder=>1])}">{GW::ln('/g/CREATE_NEW_ORDER')}</a></li>
	    <li><a class="iframeopen" href="{$m->buildUri(false,[act=>doManagePageViews,clean=>2])}" title="{GW::ln('/M/SYSTEM/MAP/childs/page_views/title')}">{GW::ln('/g/MANAGE')}</a></li>
	</ul>
    </div>	
