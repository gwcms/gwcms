...
<div class="btn-group gwFilterBtnGroup">
    ...
    <ul class="dropdown-menu">
		...
		<li class="dropdown-header">{GW::ln('/g/ADD_FILTER_4_FIELD')}</li>
        ...
        <li class="divider"></li>
        <li><a href="#" onclick="gwcms.addAllFilters();return false">{GW::ln('/g/ADD_ALL_FILTERS')}</a></li>
    </ul>
</div>	
...
<div id="gwFiltersActions" style="display:none" >	
		<button class="btn btn-default">{GW::ln('/g/APPLY_FILTER')}</button>
		...
		{if $m->list_params.filters}
			<a class="btn btn-default iframeopen float-right" href="{$m->buildUri(false,[act=>doCreatePageView,clean=>2])}"><i class="fa fa-floppy-o" title="{GW::ln('/g/CREATE_NEW_VIEW')}"></i></a>
		{/if}
		<br>
</div>
