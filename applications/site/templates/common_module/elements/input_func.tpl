...
{function name="langswitch"}
	...
	<span class="gwform_sw_ln" href="#" onclick="tooglei18nCol('{$ln_code}');return false">
			<img src="{GW::s("STATIC_EXTERNAL_ASSETS")}flags/oneimgcss/blank.gif" class="flag flag-{$flag_code}" alt="{$ln_code}" /> {if $show_ln_code_title}<span class="toggle_i18n_{$ln_code}" title="{GW::ln("/g/LANG/`$ln_code`")}">{$ln_code}</span>{/if}
	</span>
{/function}
