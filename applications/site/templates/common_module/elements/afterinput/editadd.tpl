{if $modpath}
	...
	{/if}	
	{if !$object_title}
		{$object_title=GW::ln("/M/`$tmppath.0`/MAP/childs/`$tmppath.1`/title")}
	{/if}
{/if}
...
<span class="input-group-btn addEditControls" id="addEditControls{$name}" 
	...
	{if $type!="multiselect_ajax"}
		<button class="btn btn-default editBtn" type="button" data-title="{$object_title} :: {GW::ln('/g/EDIT')}" data-shifttitle="{$object_title} :: {GW::ln('/g/LIST_CONFIG')} " data-url="{$form_url}" data-listurl="{$list_url}"><i class="fa fa-pencil-square-o"></i></button>	
	{/if}
	...
	<div class="btn-group">
		<div class="dropdown">
			...
			<ul class="dropdown-menu dropdown-menu-right" style="">
					...
					<li class="dropdown-header">{GW::ln('/g/VALUE')}</li>
					<li><a class="valueIdsBtn" type="button" title="{$object_title} :: {GW::ln('/g/VALUE')} ({GW::ln('/g/AS_DB_ID')})">{GW::ln('/g/AS_DB_ID')}</a></li>
					<li><a class="valueRowsBtn" type="button" title="{$object_title} :: {GW::ln('/g/VALUE')} ({GW::ln('/g/AS_CSV_ROWS')})">{GW::ln('/g/AS_CSV_ROWS')}</a></li>
	...
	<button class="btn btn-default addBtn" type="button" title="{$object_title} :: {GW::ln('/g/ADD')}"  data-url="{Navigator::buildURI($form_url,[id=>0])}"><i class="fa fa-plus-circle"></i></button>
