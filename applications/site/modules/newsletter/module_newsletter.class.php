...
class Module_NewsLetter extends GW_Public_Module
{
	...
	function __sendConfirmMail($subscriber)
	{
		...
		$opts = [
		    'to'=>$subscriber->email,
		    'subject'=>$this->lang['CONFIRM_SUSBSCRIBE'],
		    'body'=>$body
		];		
		
		if($this->config->default_sender)
			$opts['from'] = $this->config->default_sender;
	...
	{		
		...
		if($subscriber->validate())
		{
			...
			$response['message']=GW::ln('/m/SUCCESS_MESSAGE');
			$response['success']=1;
	...
	*/
	...
	function viewSubscribeBlock()
	{
		$this->links['subscribe']='direct/newsletter/newsletter?act=do:newsubscribe';
	}
