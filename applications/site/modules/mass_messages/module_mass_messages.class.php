...
class Module_Mass_Messages extends GW_Public_Module
{
	...
	function viewForm()
	{
		...
		if(!$item->sender)
			$item->sender = $this->app->user->last_sms_sender;
	...
	function doSendSms()
	{
		...
		{			
			...
			$this->app->setErrors($this->lang['sms_validation_failed']);
			$this->app->jump();
		}
		...
		$this->app->user->saveValues(['last_sms_sender'=>$item->sender]);

		
		$this->app->setMessage($item->status==1 ? $this->lang['sms_placed_for_sending'] : $this->lang['sms_saved']);
		$this->app->jump();
	}
	...
	function viewList()
	{
		$list = $this->model->findAll(Array('user_id=?',$this->app->user->id));
