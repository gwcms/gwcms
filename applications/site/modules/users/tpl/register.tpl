...
<section class="container  g-pt-20 g-pb-20">
		...
		<div class="row">      
			...
			<div class="col-md-6">
				...
				{input field=newsletter type=checkbox title=GW::ln("/m/SUBSCRIBE_TO_NEWSLETTER")}	
			    	...
			    	{if $smarty.session.user_link_with_fb}
				
					{capture assign="titletmp"}
						{GW::ln('/m/LINK_WITH_FB')} <b>{$smarty.session.user_link_with_fb->title}</b>
					{/capture}
					{call "input" field=link_with_fb required=0 type=checkbox title=$titletmp value=1}
		...
		{if !$smarty.session.user_link_with_fb}
                ...
                <div class="row justify-content-md-center">		
                  <div class="col-md-6">
                    <button class="btn btn-block u-btn-facebook g-font-size-12 text-uppercase g-py-12 g-px-25 mr-2" type="button" onclick="location.href='{$app->buildURI('direct/users/fblogin/redirect')}'">
