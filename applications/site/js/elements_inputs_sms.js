...
function updateDropdown(search)
{
	...
	$.get(GW.ln+'/contacts/ajax', { search:search }, 
		function(data){
...
{
	var x=$('#targets_inp').val();
	$('#targets_inp').val( x+(x?'\n':'') + target);
	$('#targets_inp').trigger('change');
}
...
{		
	$.get(GW.ln+'/contacts/ajaxByGroup', { group_id:obj.value }, 
		function(data){
			addTarget(data)
		})
